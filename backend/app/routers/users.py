...
from ..auth import get_current_user, get_password_hash, get_password_hash
from ..models import AppUser, UserRole
from ..schemas import UserCreate, UserResponse, PaginatedResponse
from ..services import get_users_by_role, create_user, update_user, delete_user
...
def create_surgeon(
    ...
    user.role = UserRole.CIRUJANO
    user.password_hash = get_password_hash(user.password)
    return create_user(db, user.dict(exclude={'password'}))
...
def create_anesthesiologist(
    ...
    user.role = UserRole.ANESTESIOLOGO
    user.password_hash = get_password_hash(user.password)
    return create_user(db, user.dict(exclude={'password'}))
